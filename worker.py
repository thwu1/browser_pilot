...
class AsyncBrowserWorker:
    ...
    async def add_task(self, task: BrowserWorkerTask):
        ...
        logger.info(f"Adding task to queue: {task.task_id} - {task.command}")
    ...
    async def process_task_queue_loop(self):
        ...
        while self.running:
            print("process task queue")
            try:
                # Wait for tasks when the queue is empty
                while self.task_queue.qsize() == 0:
                    print("Waiting for tasks..., queue size: ", self.task_queue.qsize())
                    task = await self.task_queue.get()
                    await self._execute_task(task)
                    
                # Process all available tasks without blocking
                while self.task_queue.qsize() > 0:
                    print("Processing tasks..., queue size: ", self.task_queue.qsize())
                    try:
