...
def test_context_affinity_and_browser_close():
    ...
    try:
        ...
        for context_id in first_batch_contexts:
            task_id = f"task_{uuid.uuid4().hex[:8]}"
            engine.add_task(BrowserWorkerTask(task_id=task_id, command="browser_close", context_id=context_id))
            
        # Wait for all tasks to complete
        total_tasks = 5 + 7 + 5  # First batch + Second batch + Close commands
        start_time = time.time()
        while len(engine.output_dict) < total_tasks and time.time() - start_time < max_wait:
            time.sleep(0.1)
