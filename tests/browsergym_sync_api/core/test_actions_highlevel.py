...
def test_action_parser():
    ...
    function_calls = parser.parse_string(
        'a(12, 12.2, "text", (1, 2, 3), ["a", 23])', parseAll=True
    )
    _, function_args = function_calls[0]
    assert function_args == [12, 12.2, "text", (1, 2, 3), ["a", 23]]

    function_calls = parser.parse_string(
        'a(x=12, y = 12.2, other = "text")', parseAll=True
    )
    _, function_args = function_calls[0]
    ...
    function_calls = parser.parse_string(
        'a(12, y = 12.2, other = "text")', parseAll=True
    )
    _, function_args = function_calls[0]
    assert function_args == [
        12,
    ...
    with pytest.raises(ParseException):
        function_calls = parser.parse_string(
            'a(x = 12, 12.2, other = "text")', parseAll=True
        )
    ...
    function_calls = parser.parse_string(
        'fun(12, x="val", y={"aaa": 23})', parseAll=True
    )
    function_name, function_args = function_calls[0]
...
def test_valid_action():
    ...
    env = CloudEnv(
        ...
        slow_mo=__SLOW_MO,
        timeout=__TIMEOUT,
        action_mapping={"type": "HighLevelActionSet"},
    )
...
"""
    ...
    assert (
        ...
        == "ValueError: expected a string, got [42, 'a', True, None]"
    )
...
"""
    ...
    assert (
        obs["last_action_error"]
        == "Error: Locator.click: modifiers: expected array, got string"
    )
...
def test_hover():
    ...
    env = CloudEnv(
        ...
        slow_mo=__SLOW_MO,
        timeout=__TIMEOUT,
        action_mapping={"type": "HighLevelActionSet", "subsets": ["bid", "coord"]},
    )
...
def test_scroll():
    ...
    env = CloudEnv(
        ...
        slow_mo=__SLOW_MO,
        timeout=__TIMEOUT,
        action_mapping={"type": "HighLevelActionSet", "subsets": ["coord"]},
    )
...
def test_tab_actions():
    ...
    env = CloudEnv(
        ...
        slow_mo=__SLOW_MO,
        timeout=__TIMEOUT,
        action_mapping={"type": "HighLevelActionSet", "subsets": ["tab", "nav"]},
    )
    obs, info = env.reset()
    assert not obs["last_action_error"]
    ...
    obs, reward, term, trunc, info = env.step("new_tab()")
    assert not obs["last_action_error"]
    ...
    obs, reward, term, trunc, info = env.step(f"goto({repr(TEXTBOX_URL)})")
    assert not obs["last_action_error"]
    assert len(obs["open_pages_urls"]) == 2
    assert len(obs["open_pages_titles"]) == 2
    assert obs["active_page_index"][0] == 1
    ...
    obs, reward, term, trunc, info = env.step("tab_focus(0)")
    assert not obs["last_action_error"]
    assert len(obs["open_pages_urls"]) == 2
    assert len(obs["open_pages_titles"]) == 2
    assert obs["active_page_index"][0] == 0
    ...
    obs, reward, term, trunc, info = env.step("tab_close()")
    assert not obs["last_action_error"]
    assert len(obs["open_pages_urls"]) == 1
    assert len(obs["open_pages_titles"]) == 1
    assert obs["active_page_index"][0] == 0
...
def test_iframe_bid():
    ...
    env = CloudEnv(
        ...
        slow_mo=__SLOW_MO,
        timeout=__TIMEOUT,
        action_mapping={"type": "HighLevelActionSet", "subsets": ["bid"]},
    )
    obs, info = env.reset()
