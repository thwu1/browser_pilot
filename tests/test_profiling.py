...
def calculate_metrics(outputs):
    worker_recv_to_start_process = [output["profile"]["worker_start_process_timestamp"] - output["profile"]["worker_recv_timestamp"] for output in outputs]
    assert all([x >= 0 for x in worker_recv_to_start_process]), f"worker_recv_to_start_process: {worker_recv_to_start_process}"
    worker_start_process_to_finish = [output["profile"]["worker_finish_timestamp"] - output["profile"]["worker_start_process_timestamp"] for output in outputs]
    worker_finish_to_send = [output["profile"]["worker_send_timestamp"] - output["profile"]["worker_finish_timestamp"] for output in outputs]
    ...
    def stats(arr, name):
        ...
        if not arr:
            ...
            return
        print(f"{name}: avg={sum(arr)/len(arr):.4f}s, min={min(arr):.4f}s, max={max(arr):.4f}s, p95={sorted(arr)[int(0.95*len(arr))-1]:.4f}s")
    ...
    total_times = [recv + start + send for recv, start, send in zip(worker_recv_to_start_process, worker_start_process_to_finish, worker_finish_to_send)]
    ...
    print(f"Percentage breakdown: recv_to_start={recv_pct:.2f}%, start_to_finish={start_pct:.2f}%, finish_to_send={send_pct:.2f}%")
    print("--------------------------")

    return worker_recv_to_start_process, worker_start_process_to_finish, worker_finish_to_send
...
def analyze_by_command(outputs):
    ...
    for cmd, (recv, start, fin) in stats.items():
        print(f"{cmd}: recv->start avg={sum(recv)/len(recv):.4f}s, start->finish avg={sum(start)/len(start):.4f}s, finish->send avg={sum(fin)/len(fin):.4f}s")
...
def test_profiling_metrics():
    ...
    async def go_to_youtube_bilibili_return_html():
        ...
        output, page_id = await goto_url_helper(client, context_id, None, "https://www.youtube.com")
        outputs.append(output)
        assert output["result"]["success"] == True

        output, page_id = await get_observation_helper(client, context_id, page_id, "accessibility")
        outputs.append(output)
        assert output["result"]["success"] == True

        output, page_id = await goto_url_helper(client, context_id, page_id, "https://www.bilibili.com")
        outputs.append(output)
        assert output["result"]["success"] == True

        output, page_id = await get_observation_helper(client, context_id, page_id, "html")
        outputs.append(output)
        assert output["result"]["success"] == True

        output, page_id = await goto_url_helper(client, context_id, page_id, "https://www.nytimes.com")
        outputs.append(output)
        assert output["result"]["success"] == True
        ...
        output, page_id = await goto_url_helper(client, context_id, page_id, "https://www.reddit.com")
        outputs.append(output)
        assert output["result"]["success"] == True
        ...
        output, page_id = await goto_url_helper(client, context_id, page_id, "https://www.amazon.com")
        outputs.append(output)
        assert output["result"]["success"] == True
        ...
        output, page_id = await goto_url_helper(client, context_id, page_id, "https://www.apple.com")
        outputs.append(output)
        assert output["result"]["success"] == True
    ...
    async def main():
        ...
        outputs = await asyncio.gather(*[go_to_youtube_bilibili_return_html() for _ in range(16)])
        end_time = time.time()
        ...
        recv_to_start, start_to_finish, finish_to_send = calculate_metrics(outputs_flattened)
        plt.figure(figsize=(10, 6))
        plt.boxplot([recv_to_start, start_to_finish, finish_to_send], labels=["recv_to_start", "start_to_finish", "finish_to_send"])
        plt.ylabel("Time (s)")
