...
def test_round_robin_scheduler_with_existing_assignments():
    ...
    tasks = [
        BrowserWorkerTask(task_id=f"task_{i}", command="test_command") for i in range(3)
    ]
    ...
    scheduled_tasks, output = scheduler.schedule(
        tasks, last_assigned_workers, 3, worker_status
    )
...
def test_scheduler_stress():
    ...
    # Schedule all tasks
    scheduled_tasks, output = scheduler.schedule(
        tasks, last_assigned_workers, 5, worker_status
    )
