...
class StatusTracker:
    ...
    def __init__(self, endpoints: List[str], is_running: bool = True):
        ...
        self.endpoint_websocket_stats = defaultdict(
            lambda: {
    ...
    def register_connection(self, endpoint: str):
        if not self.is_running:
            return
        self.endpoint_websocket_stats[endpoint]["active_connections"] += 1
    ...
    def get_status(self) -> Dict:
        ...
        return {"task": endpoint_stats, "websocket": self.endpoint_websocket_stats}

    def unregister_connection(self, connection_id: str, endpoint: str, reason: str):
        """Clean up all tasks for a connection when it closes"""
        if not self.is_running:
            return
        assert reason in ["finished", "error"]
        self.endpoint_websocket_stats[endpoint]["active_connections"] -= 1
        self.endpoint_websocket_stats[endpoint]["finished_connections"] += 1
        if reason == "error":
            self.endpoint_websocket_stats[endpoint]["error_connections"] += 1
