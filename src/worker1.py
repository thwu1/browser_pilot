...
class AsyncBrowserWorker:
    ...
    async def background_process_loop(self):
        """Process tasks from the queue in the background"""
        while self.running:
            logger.debug("process task queue")
            try:
                ...
                while self.input_queue.qsize() > 0:
                    logger.debug(f"Processing tasks..., queue size: {self.input_queue.qsize()}")
                    try:
    ...
    async def _execute_task(self, task: Dict):
        ...
        logger.debug(f"put success result in queue, input queue size: {self.input_queue.qsize()}, output queue size: {self.output_queue.qsize()}")
    ...
    # async def execute_command(self, context_id: str, command: str, params: Dict[str, Any] = None) -> Dict[str, Any]:
    #     """
    ...
    #             # Navigate to URL
    #             page = await self._get_or_create_page(context_id, params.get("page_id"))
    ...
    #             result = {"success": True, "pdf": pdf_data}
    ...
    #             result = {"success": True, "result": eval_result}
...
class AsyncBrowserWorkerProcess:
    ...
    def __init__(self, worker_id: str, input_path: str, output_path: str):
        ...
        self.input_socket = make_zmq_socket(self.ctx, input_path, zmq.PULL, bind=True)
        self.output_socket = make_zmq_socket(self.ctx, output_path, zmq.PUSH, bind=True)

        self.worker = AsyncBrowserWorker(worker_id)
        self.logger = logging.getLogger("AsyncBrowserWorkerProcess")
    ...
    def run_background_loop(worker_id: str, input_path: str, output_path: str):
        ...
        #     format='%(asctime)s - %(name)s - %(levelname)s - %(message)s'
        # )
        proc = AsyncBrowserWorkerProcess(worker_id, input_path, output_path)
        
        async def main_loop():

            await proc.worker.start()
            proc.logger.info(f"Worker started with input path = {input_path}, output path = {output_path}")

            tasks = [proc.process_incoming_socket_loop(), proc.worker.background_process_loop(), proc.process_outgoing_socket_loop()]
    ...
    async def process_incoming_socket_loop(self):
        ...
        while self.worker.running:
            logger.debug("process incoming socket")
            try:
                task = self.input_socket.recv_json(flags=zmq.NOBLOCK)
                self.worker.input_queue.put_nowait(task)
                logger.debug(f"Received task:  {task} added to queue, input queue size {self.worker.input_queue.qsize()}, output queue size {self.worker.output_queue.qsize()}")
            except zmq.Again:
                logger.debug("No message to receive")
                await asyncio.sleep(0.1)
            except Exception as e:
                logger.error(f"During process_incoming_socket_loop, encouter: {str(e)}")
                await asyncio.sleep(0.1)  # Longer sleep on error
            
    async def process_outgoing_socket_loop(self):
        logger = self.logger
        while self.worker.running:
            logger.debug(f"process outgoing socket, input queue size: {self.worker.input_queue.qsize()}, output queue size: {self.worker.output_queue.qsize()}")
            output = await self.worker.output_queue.get()
            self.output_socket.send_json(output)
            ...
            # self.output_socket.send_json(output)
            # logger.debug(f"AsyncBrowserWorkerProcess sent {output}")
