...
from async_engine_v2 import AsyncBrowserEngine
...
if __name__ == "__main__":
    ...
    loop.run_until_complete(engine.worker_client._wait_for_workers_ready())

    loop.create_task(engine.engine_core_loop())
    loop.create_task(engine.process_incoming())
    loop.create_task(engine.process_output())
    loop.create_task(engine.worker_client._recv_loop())
