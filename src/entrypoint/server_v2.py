...
async def lifespan(_: FastAPI):
    ...
    for _ in range(256):
        sockets.put_nowait(make_zmq_socket(ctx, "ipc://app_to_engine.sock", zmq.DEALER, bind=False, identity=str(uuid.uuid4().hex[:10]).encode()))
    logger.info(f"Created {256} sockets for engine")
    try:
        yield
...
async def start(config: Dict[str, Any] = None):
    ...
    lock_path = "start_engine_lock.lock"

    # this code runs in every worker…
    with open(lock_path, "w") as lock_fp:
        try:
            # try to grab an exclusive, non‐blocking lock
            fcntl.flock(lock_fp, fcntl.LOCK_EX | fcntl.LOCK_NB)
        except BlockingIOError:
            # someone else already has the lock → skip starting engine
            pass
        else:
            # we got the lock → spin up exactly one engine
            engine = mp.Process(
                target=AsyncBrowserEngine.spin_up_engine,
                args=(engine_config,)
            )
            engine.start()
...
if __name__ == "__main__":
    ...
    if "root" not in LOGGING_CONFIG or LOGGING_CONFIG["root"] is None:
        LOGGING_CONFIG["root"] = {"handlers": [], "level": "INFO"}
    ...
    try:
        # Run the server
        # Use server.run() so the `workers=4` setting is applied and multiple processes are spawned
        server.run()
    except asyncio.CancelledError:
